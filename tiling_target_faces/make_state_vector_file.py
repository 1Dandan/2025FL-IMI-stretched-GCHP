...
def make_state_vector_file(
    ...
    if UseGCHP:
        prefix = get_gridspec_prefix(CS_RES, True, STRETCH_FACTOR, TARGET_LAT, TARGET_LON)
        gridfpath = f'{CSgridDir}{prefix}_formatted.nc'
        gridds = xr.open_dataset(gridfpath).squeeze()
        lons = gridds['lons'].values.copy()
        lons[lons>180] -= 360
        lats = gridds['lats'].values.copy()
        # convert to CS_grids
        if Met in ["GEOSFP", "GEOS-FP", "geosfp"]:
            nativeres = "0.25x0.3125"
            nlats = 721
            nlons = 1152
        elif Met in ["MERRA2", "MERRA-2", "merra2"]:
            nativeres="0.5x0.625"
            nlats = 361
            nlons = 576
        else:
            print("\nERROR: Meteorology field {} is not supported by the IMI. ".format(Met))
            print("\n Options are GEOSFP or MERRA2.\n")
        if STRETCH_GRID:
            transform = sparselt.esmf.load_weights(
                f"{CSgridDir}/regrid_weights_{nativeres}_to_c{CS_RES}_s{STRETCH_FACTOR}_{TARGET_LAT}N_{TARGET_LON}E_conserve.nc",   # Generated by ESMF_RegridWeightGen
                output_dims=[('nf', 'Ydim', 'Xdim'), (6, CS_RES, CS_RES)],  # output dimensions
                input_dims=[('lat', 'lon'), (nlats, nlons)],           # input dimensions
            )
        else:
            transform = sparselt.esmf.load_weights(
                f"{CSgridDir}/regrid_weights_{nativeres}_to_c{CS_RES}_conserve.nc",   # Generated by ESMF_RegridWeightGen
                output_dims=[('nf', 'Ydim', 'Xdim'), (6, CS_RES, CS_RES)],  # output dimensions
                input_dims=[('lat', 'lon'), (nlats, nlons)],           # input dimensions
            )
    ...
    if UseGCHP:
        ...
        if STRETCH_GRID:
            ds_statevector.attrs['STRETCH_FACTOR'] = np.float32(STRETCH_FACTOR)
            ds_statevector.attrs['TARGET_LAT'] = np.float32(TARGET_LAT)
            ds_statevector.attrs['TARGET_LON'] = np.float32(TARGET_LON)
    else:
